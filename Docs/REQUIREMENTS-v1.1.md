...
| `birthTime` | 必須（`null` 可） | `HH:mm` または `HH:mm:ss`。**暦・時刻**は `context.timeZone` の**民用標準時**（暦・時刻 **4**）。キー自体は常に送信し、未知時刻は `null` を指定する。**節入り日**（マスタ上、その節の入り瞬間を TZ 投影した暦日）と `birthDate` が一致する場合は `null` 不可（§7、[OPEN-QUESTIONS.md](./OPEN-QUESTIONS.md) 暦・時刻 **1**）。 |
| `timeZoneId` | 必須 | IANA TZ。`context.timeZone` と同値であること（不一致は `VALIDATION_ERROR`）。 |
...
- **入力契約（sanmei-core v1）**: `birthTime` は「条件付きキー」ではなく**常にキー必須**（値は `string | null`）。未確定時は `null` を送る。さらに `user.timeZoneId` と `context.timeZone` は**一致必須**とする。
...
太陽黄経から毎リクエスト計算するのではなく、節入り瞬間を**分単位**（または秒）で保持した静的データを `sanmei-core` に同梱する。**現在同梱しているマスタの年範囲は 1990〜2035**（`solar_terms.json` の `meta.rangeStartYear` / `meta.rangeEndYear`）で、将来は 1900〜2100 へ拡張可能。ランタイムでは UTC 等の絶対瞬間として解釈し、リクエストのローカル日時と比較する（§5「出生時刻の解釈」）。
...
- **推奨手順**: **ビルド時（オフライン）**に節入り瞬間マスタを**分・秒単位**で生成する。現行同梱は 1990〜2035 だが、要件上の拡張目標は 1900〜2100（必要ならさらに拡張）。Rust のバインディング等で生成ツールを実装し、**TS／Rust のランタイムは同一のアーティファクト**（JSON／SQLite 等）を読み込む（計算系の完全統一）。
...
| v1.1.7 | §5: `user.timeZoneId` を必須化し `context.timeZone` 一致要件を明文化。`birthTime` は「キー必須・値 null 可」に明確化。§9: 同梱節入りマスタの現行年範囲（1990〜2035）と拡張目標（1900〜2100）を併記 |
