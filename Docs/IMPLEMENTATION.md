...
- Ruleset: [bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts)（研究ブロック共有は [bundledResearchV1Shared.ts](../packages/sanmei-core/src/layer2/bundledResearchV1Shared.ts)）＋`bundledMockRulesetV1`（後方互換）、Zod は `schemas/rulesetMockV1.ts`（`BundledRulesetSchema`＝`mock-v1`／`mock-internal-v2`／`research-v1`／**`research-experimental-v1`** の `z.union`、`interaction.isouhouExtended` / `locatorPriority` 任意、`timelineMock`、`meta.schemaRevision`、`energyWeights` 等）。欠セルは `RULESET_DATA_MISSING`。
- レスポンス契約: `schemas/layer2.ts`（`baseProfile` は従来どおり＋**`dynamicTimeline`**（`daiun`／`annual` 等）。`daiun.phases[]` は `interactions?`（phase 単位の位相法マトリクス表示用）を持ちうる。**`interactionRules`** に `isouhou`・`kyoki`・`resolutionMeta?`・`kaku?` を持つ；`guardianDeities`／`kishin` は五行コード列＝`Element` 数値）。**`meta.warnings?`** は任意（experimental ruleset 等）。
...
| Layer2 | [layer2/](../packages/sanmei-core/src/layer2/)、[schemas/layer2.ts](../packages/sanmei-core/src/schemas/layer2.ts)、[schemas/rulesetMockV1.ts](../packages/sanmei-core/src/schemas/rulesetMockV1.ts)、[src/data/rulesets/](../packages/sanmei-core/src/data/rulesets/) | 蔵干・主星・従星・守護神忌神・六親・**L2c**・**`resolveDynamicTimeline`**・**`applyLayer3aByRuleset`**・**`applyLayer3bByRuleset`**（`mock-*` は noop、研究系は位相法+格法。experimental は拡張5種+timeline）・[bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts)・[bundledResearchV1Shared.ts](../packages/sanmei-core/src/layer2/bundledResearchV1Shared.ts)。§2 参照 |
