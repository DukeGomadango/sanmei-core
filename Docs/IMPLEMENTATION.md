...
- Orchestrator: [calculate.ts](../packages/sanmei-core/src/calculate.ts)— `CalculateInputSchema` → `requiresBirthTimeForAnySolarTermOnDate` → `resolveInsenWithDepth` → **`getBundledRuleset(rulesetVersion)`**（`mock-v1` / `mock-internal-v2` / `research-v1` / **`research-experimental-v1`**）→ 蔵干・主星・従星・守護神／忌神・六親（`familyNodes` 座標付き）→ **L2c** `resolveEnergyData`／`resolveDestinyBugs` → **`dynamicTimeline`**（`.resolveDynamicTimeline`。`mock-*` は満年齢ベース、`research-v1` と **`research-experimental-v1`** は大運R1: 順逆判定 + JDN日数差 + 3除算四捨五入 + 0/11補正）→ **Layer3a resolver** `applyLayer3aByRuleset`（`rulesetVersion` ディスパッチ。`research-v1` / **`research-experimental-v1`** は位相法を返し、`mock-*` は noop。experimental は `isouhouExtended` を Layer3a で基準8種と統合ソート）→ **Layer3b resolver** `applyLayer3bByRuleset`（研究系は `kaku` を返却、`mock-*` は noop）。`user.timeZoneId` と `context.timeZone` の一致を要求（REQUIREMENTS の民用 TZ 基準）。**`research-experimental-v1`** のとき `meta.warnings` に L2 注意文を付与。
- Ruleset: [bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts)（研究ブロック共有は [bundledResearchV1Shared.ts](../packages/sanmei-core/src/layer2/bundledResearchV1Shared.ts)）＋`bundledMockRulesetV1`（後方互換）、Zod は `schemas/rulesetMockV1.ts`（`BundledRulesetSchema`＝`mock-v1`／`mock-internal-v2`／`research-v1`／**`research-experimental-v1`** の `z.union`、`starLabels?`（研究系は [researchStarLabels.ts](../packages/sanmei-core/src/layer2/researchStarLabels.ts) 由来の全行列 ID カタログ）、**`tenchuRules?`（`b2`＝`TenchuRulesB2Schema`・`research-tenchu-b2-v1`）**、`interaction.isouhouExtended` / `locatorPriority` 任意、`timelineMock`、`meta.schemaRevision`、`energyWeights` 等）。欠セルは `RULESET_DATA_MISSING`。
- **研究系の応答（starLabels / 天中殺 B1+B2）**: `calculate` は `ruleset.starLabels` から **主星5＋従星3** に出た `starId` のみを **`meta.display.starLabels`** に載せる。`research-*` では [resolveTenchuSatsuStatus.ts](../packages/sanmei-core/src/layer2/resolveTenchuSatsuStatus.ts) で B1（`destinyBugRules`）に加え、**`tenchuRules.b2`（`dslVersion: research-tenchu-b2-v1`）** がある場合は年運・大運の **六十甲子 index 集合照合**を `dynamic` にマージ（`phase: B2_DYNAMIC`）。詳細は [PLAN-RESEARCH-YOUSEN-AND-TENCHU.md](./PLAN-RESEARCH-YOUSEN-AND-TENCHU.md)・[RESEARCH-SECT-SPEC.md](./RESEARCH-SECT-SPEC.md) §4.5。
- レスポンス契約: `schemas/layer2.ts`（`baseProfile` は従来どおり＋**`dynamicTimeline`**（`daiun`／`annual`／**`tenchuSatsuStatus?`** 等）。`daiun.phases[]` は `interactions?`（phase 単位の位相法マトリクス表示用）を持ちうる。**`interactionRules`** に `isouhou`・`kyoki`・`resolutionMeta?`・`kaku?` を持つ；`guardianDeities`／`kishin` は五行コード列＝`Element` 数値）。**`meta.warnings?`**・**`meta.display?`** は任意（experimental ruleset・研究 starLabels 等）。
...
- **バンドル ruleset**: サポートは **`BUNDLED_RULESET_VERSIONS`**（例: `mock-v1`、`mock-internal-v2`、`research-v1`、**`research-experimental-v1`**）。レジストリに無い版は `RULESET_VERSION_UNSUPPORTED`。BFF は本番と同形のリクエストを送れる。
...
- **Layer3a（M2）**: `applyLayer3aByRuleset`（resolver ディスパッチ）。`research-v1` は ruleset の `interaction`（`patterns`/`priorityOrder`）で `isouhou` を判定、`HANKAI` に `hasCentralBranch` を付与、`kyoki` は gate で `null` 維持。**`research-experimental-v1`** は上に加え `isouhouExtended`（`KANGO`/`NACHION`/`RITSUON`/`TENKOKUCHICHU`/`DAIHANKAI`）を命式3柱の柱ペアで検出し、**基準8種より後**にソート（`conflictPolicy.baseKindsFirst`）。天剋地冲の天干部は `layer1/stemPairInteractions.ts`（`areStemsTenkokuPair`）。`mock-v1` / `mock-internal-v2` は `applyLayer3aMock` の noop を維持。
- **Layer3b（M3）**: `applyLayer3bByRuleset`（resolver ディスパッチ）。`research-v1` / **`research-experimental-v1`** は `kaku.candidates[]`（全件）→ `kaku.resolved[]` / `kaku.suppressed[]` を返し、`allowGohouInKaku` を反映。格法の `requiresIsouhouKindsAny` は基準種＋**拡張種**（Zod: `IsouhouAnyKindSchema`）。`kaku.meta` に `allowGohouInKakuApplied` / `evaluateShadowProfileApplied` を保持。`mock-v1` / `mock-internal-v2` は noop を維持。
...
| Playground SPA | [packages/sanmei-playground/](../packages/sanmei-playground/) | 管理者用の Vite+React+TS SPA。`POST /api/v1/calculate` の結果を**セクション型ダッシュボード**で可視化: 計算メタ（`meta.warnings` 含む）、静的命式（`insen`＋蔵干・`rawDelta`/`displayDepth` 説明、`yousen`、`destinyBugs`、`familyNodes`、`energyData` の total/五行）、動的運勢（`daiun` の順逆行・干支・フェーズ毎 `interactions`、`annual`、`tenchuSatsuStatus`）、ルール解決（`isouhou` 詳細＋`resolutionMeta`、`kaku`）。入力は `allowGohouInKaku` トグル、`TIME_REQUIRED_FOR_SOLAR_TERM` 時は `error.details`（節入り瞬間）を選択 TZ で整形した案内を表示。型は `@sanmei/sanmei-core` の export（`CalculateResult` 等）を正とし、家族座標は `familyNodeDisplay` で整形。`CalculationTraceTabs`（Layer1/2/3 + Raw JSON、アンカー導線）で `debugTrace` を表示。**ブラウザ bundler**: 実行時 import で `@sanmei/sanmei-core` のメインを引くと `layer1/solarTerms/loadJson`（`node:fs`）まで結線され得るため、`indexToStemBranch` 等の純関数は `@sanmei/sanmei-core/sexagenary` を使う（`package.json` の `exports`）。**UI運用メモ**: カード内余白は原則 `p-5`、タイムライン文言は `whitespace-nowrap`。 |
| 公開 API | [index.ts](../packages/sanmei-core/src/index.ts)、`package.json` `exports` | 再エクスポート集約（`calculate`・`SanmeiError`・Layer2 Zod 型など）。ブラウザ向けには **`./sexagenary`**（`layer1/sexagenary` のみ、fs 非依存）。 |
...
| Layer2 | [layer2/](../packages/sanmei-core/src/layer2/)、[schemas/layer2.ts](../packages/sanmei-core/src/schemas/layer2.ts)、[schemas/rulesetMockV1.ts](../packages/sanmei-core/src/schemas/rulesetMockV1.ts)、[src/data/rulesets/](../packages/sanmei-core/src/data/rulesets/) | 蔵干・主星・従星・守護神忌神・六親・**L2c**・**`resolveDynamicTimeline`**・**`applyLayer3aByRuleset`**・**`applyLayer3bByRuleset`**（`mock-*` は noop、研究系は位相法+格法。experimental は拡張5種+timeline）・[bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts)・[bundledResearchV1Shared.ts](../packages/sanmei-core/src/layer2/bundledResearchV1Shared.ts)・研究用星名 [researchStarTables.ts](../packages/sanmei-core/src/layer2/researchStarTables.ts)／[researchDestinyBugRulesSubset.ts](../packages/sanmei-core/src/layer2/researchDestinyBugRulesSubset.ts)。§2 参照 |
...
| Primitives | `layer1/enums.ts`, `stemBranchTables.ts`, `wuxingRelations.ts`, `kango.ts`, **`stemPairInteractions.ts`**（地支冲・天剋干判定）, `sexagenary.ts` | DOMAIN-GLOSSARY Layer1 に対応。 |
...
| テスト | `*.test.ts`、`__tests__/goldenHarness.ts` | Vitest。ルート `npm test` は **全 workspace** の `test` を実行。研究用の主星・従星公開表突合は [researchStarMatrixDiff.test.ts](../packages/sanmei-core/src/layer2/researchStarMatrixDiff.test.ts)（[RESEARCH-STAR-MATRIX-DIFF-ITER28.md](./RESEARCH-STAR-MATRIX-DIFF-ITER28.md)）。天中殺 B1 の柱キー照合デモは [resolveTenchuSatsuStatusB1.test.ts](../packages/sanmei-core/src/layer2/resolveTenchuSatsuStatusB1.test.ts)。 |
...
- **配置（ソース）**: `packages/sanmei-core/src/data/rulesets/*.json`（`mock-v1.json`、`mock-internal-v2.json`＝レジストリ検証用・本文は mock と同形）。`research-v1` / **`research-experimental-v1`** は [bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts) で `mock-v1` 本文を基底にしつつ、`interaction`（位相法・格法）に加えて `researchDaiun`（大運R1契約）を注入。experimental は共有ブロック [bundledResearchV1Shared.ts](../packages/sanmei-core/src/layer2/bundledResearchV1Shared.ts) に `isouhouExtended`・`locatorPriority`・追加 `kaku` 規則を上乗せ。
...
- **Zod**: `BundledRulesetSchema`＝`z.union`（将来 **discriminatedUnion** 布石）。`mock-v1` の **`meta.schemaRevision`** は **2**（`timelineMock` 追加）。`research-v1` は Layer3a/3b 契約（`interaction.patterns` / `interaction.kaku`）を含む実装段階。**`research-experimental-v1`** は `meta` 字句別＋必須 `interaction.isouhouExtended`（拡張5種・`scopePolicy`・`conflictPolicy`）を付す並行バンドル（`research-v1` の互換は別キー）。
...
- **研究流派の今後の拡張（計画）**: 十大主星・十二大従星の **starId→表示名カタログ**（バンドル内の全 ID は CI で行列と集合一致、**API 既定では結果に出た ID のみを `meta.display` に載せる方針**）と **`dynamicTimeline.tenchuSatsuStatus` 算出**のスプリント分割は [PLAN-RESEARCH-YOUSEN-AND-TENCHU.md](./PLAN-RESEARCH-YOUSEN-AND-TENCHU.md) を正とする（**完了条件の逆算・G0〜G2** は同書 §7、[RESEARCH-SECT-RULESET-WORKFLOW.md](./RESEARCH-SECT-RULESET-WORKFLOW.md) の実装ゲートと併読）。固有名の漢字ゆれ・`destinyBugRules`（B1）キー対応は同ワークフロー **18.10**／**18.11**。従星セル突合ログは [RESEARCH-STAR-MATRIX-DIFF-ITER28.md](./RESEARCH-STAR-MATRIX-DIFF-ITER28.md)。`takao-v1` 本番路線は [PHASE2-RULESET-AND-DAIUN.md](./PHASE2-RULESET-AND-DAIUN.md)。
