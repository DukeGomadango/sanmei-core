...
- レスポンス契約: `schemas/layer2.ts`（`baseProfile` は従来どおり＋**`dynamicTimeline`**（`daiun`／`annual` 等）。**`interactionRules`** に `isouhou`・`kyoki`・`resolutionMeta?` を持つ；`guardianDeities`／`kishin` は五行コード列＝`Element` 数値）。
...
| Layer2 | [layer2/](../packages/sanmei-core/src/layer2/)、[schemas/layer2.ts](../packages/sanmei-core/src/schemas/layer2.ts)、[schemas/rulesetMockV1.ts](../packages/sanmei-core/src/schemas/rulesetMockV1.ts)、[src/data/rulesets/](../packages/sanmei-core/src/data/rulesets/) | 蔵干・主星・従星・守護神忌神・六親・**L2c**・**`resolveDynamicTimeline`**・**`applyLayer3aByRuleset`**（`mock-*` は noop、`research-v1` は位相法）・[bundledRulesets.ts](../packages/sanmei-core/src/layer2/bundledRulesets.ts)。§2 参照 |
