...
| 十大主星: 決定軸（日干×対象干） | `takao-main-stars-matrix-v1` | `shugakuin-main-stars-matrix-v1` | `research-main-stars-vector-logic-v1` (PENDING-VERIFICATION, sourceLevel: L2) | 五行関係＋陰陽で主星を決定。`research` は独立2系統で算出軸を再確認済み、セル値固定は `L0` 待ち。固有名の漢字正規化は [RESEARCH-SECT-RULESET-WORKFLOW.md](./RESEARCH-SECT-RULESET-WORKFLOW.md) **18.10.1**。 |
| 十二大従星: 決定軸（日干×地支） | `takao-sub-stars-matrix-v1` | `shugakuin-sub-stars-matrix-v1` | `research-sub-stars-daystem-branch-v1` (PENDING-VERIFICATION, sourceLevel: L2) | 日干×地支で従星を決定。表記揺れの正規化は [RESEARCH-SECT-RULESET-WORKFLOW.md](./RESEARCH-SECT-RULESET-WORKFLOW.md) **18.10.1**。セル突合ログは [RESEARCH-STAR-MATRIX-DIFF-ITER28.md](./RESEARCH-STAR-MATRIX-DIFF-ITER28.md)。 |
...
| 天中殺 B1（宿命表） | `takao-tenchu-b1-table-v1` | `shugakuin-tenchu-b1-table-v1` | `research-tenchu-b1-table-catalog-v1` (PENDING-VERIFICATION, sourceLevel: L2) | 宿命側は出生で凍結。4 機械キーに Stock 個別ページを二次紐付け済み（[WORKFLOW 18.11](./RESEARCH-SECT-RULESET-WORKFLOW.md)）。表本文は `L0` 待ち。 |
| 年運天中殺: 周期・期間 | **共通（例: 12 年周期／2 年間）** | **共通（例: 12 年周期／2 年間）** | `research-tenchu-b2-slide-dsl-v1` (PENDING-MEDIUM, sourceLevel: L2) | 起算・境界（例: 立春基準の説がある）や「2 年」の数え方は **【要監修】**。表の「共通」は仮ラベル。 |
| 大運天中殺: 期間・スライド有無 | **共通 (YES)** | **共通 (YES)** | `research-tenchu-b2-slide-dsl-v1` (PENDING-MEDIUM, sourceLevel: L2) | 大運天中殺の有無・基本スパン（例: 20 年＝2 旬）の扱い。**スライド条件は DSL で管理**。[OPEN-QUESTIONS.md](./OPEN-QUESTIONS.md) §8。 |
